...
def upload_notes(request):
    ...
    if request.method=='POST':
        ...
        try:
            Notes.objects.create(user=u,uploadingdate=date.today(),branch=b,subject=s,notesfile=n,filetype=f,description=de,status='pending')
            error="no"
        except:
            error="yes"
    d = {'error': error}
...
#
#     if query:
#         # If a query is provided, filter notes based on keywords
#         notes = Notes.objects.filter(Q(status="Accept") & (Q(branch__icontains=query) | Q(subject__icontains=query) | Q(description__icontains=query)))
#     else:
#         # If no query is provided, display all notes with status "Accept"
#         notes = Notes.objects.filter(status="Accept")
#
#     d = {'notes': notes}
#     return render(request, 'view_all_notes.html', d)
...
def view_all_notes(request):
    ...
    if query:
        # If a query is provided, filter notes based on keywords, excluding filetype and status
        notes = Notes.objects.filter(
            Q(branch__icontains=query) | Q(subject__icontains=query) | Q(description__icontains=query)
        ).filter(status="Accept")  # Exclude notes with status other than "Accept"
    else:
        # If no query is provided, display all notes with status "Accept"
        notes = Notes.objects.filter(status="Accept")
